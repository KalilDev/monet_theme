...
var intFromRgb = rgb => (-16777216 | (rgb[0] & 255) << 16 | (rgb[1] & 255) << 8 | rgb[2] & 255) >>> 0;
var labFromInt = argb => {
    const e = 216 / 24389, kappa = 24389 / 27, redL = 100 * google3.linearized(((argb & 16711680) >> 16) / 255), greenL = 100 * google3.linearized(((argb & 65280) >> 8) / 255), blueL = 100 * google3.linearized((argb & 255) / 255), yNormalized = (0.2126 * redL + 0.7152 * greenL + 0.0722 * blueL) / google3.WHITE_POINT_D65[1];
    let fy;
    fy = yNormalized > e ? Math.pow(yNormalized, 1 / 3) : (kappa * yNormalized + 16) / 116;
    const xNormalized = (0.41233895 * redL + 0.35762064 * greenL + 0.18051042 * blueL) / google3.WHITE_POINT_D65[0], zNormalized = (0.01932141 * redL + 0.11916382 * greenL + 0.95034478 * blueL) / google3.WHITE_POINT_D65[2];
    return [
        116 * fy - 16,
...
var intFromXyzComponents = (x, y, z) => {
    ...
    y /= 100;
    z /= 100;
    return google3.intFromRgb([
        Math.round(utils.math_utils_clamp(255, 255 * google3.delinearized(3.2406 * x + -1.5372 * y + -0.4986 * z))),
        Math.round(utils.math_utils_clamp(255, 255 * google3.delinearized(-0.9689 * x + 1.8758 * y + 0.0415 * z))),
        Math.round(utils.math_utils_clamp(255, 255 * google3.delinearized(0.0557 * x + -0.204 * y + 1.057 * z)))
    ]);
};
var intFromHex = hex => {
    hex = hex.replace('#', '');
