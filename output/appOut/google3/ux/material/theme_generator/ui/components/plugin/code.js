...
function tabs_pluginTabs(context) {
    ...
    ${ components.chip_button_chipButton({
        ...
        callback: () => {
            ...
            const oldTheme = context.theme, seedColor = oldTheme.save().source.primary, theme = oldTheme.isBaseline ? google3.default(!0) : google3.fromColor(seedColor, !0);
            context.updateTheme(theme);
        }
    }) }
    ${ components.chip_button_chipButton({
        ...
        callback: () => {
            ...
            const oldTheme = context.theme, themeSnapshot = oldTheme.save(), seedColor = themeSnapshot.source.seed, theme = oldTheme.isBaseline ? google3.default(!0) : google3.fromColor(seedColor, !0, { source: themeSnapshot.source });
            context.updateTheme(theme);
        }
    }) }
  </div>`;
