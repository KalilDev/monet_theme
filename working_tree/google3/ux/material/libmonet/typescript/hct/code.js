...
function hct_getIntInViewingConditions(hue$jscomp$0, chroma$jscomp$0, tone$jscomp$0) {
    ...
    for (; 0.4 <= Math.abs(low - high);) {
        ...
        for (; 0.01 < Math.abs(low$jscomp$0 - high$jscomp$0);) {
            ...
            const clipped = JSCompiler_StaticMethods_viewed(google3.fromJchInViewingConditions(mid$jscomp$0, chroma, hue)), clippedLstar = google3.lstarFromInt(clipped), dL = Math.abs(tone - clippedLstar);
            if (0.2 > dL) {
                const camClipped = google3.fromIntInViewingConditions(clipped), dE = camClipped.distance(google3.fromJchInViewingConditions(camClipped.j, camClipped.chroma, hue));
        ...
        if (isFirstLoop) {
            if (null != possibleAnswer)
                return JSCompiler_StaticMethods_viewed(possibleAnswer);
            isFirstLoop = !1;
        } else
            null === possibleAnswer ? high = mid : (answer = possibleAnswer, low = mid);
        mid = low + (high - low) / 2;
    }
    return null === answer ? google3.intFromLstar(tone$jscomp$0) : JSCompiler_StaticMethods_viewed(answer);
}